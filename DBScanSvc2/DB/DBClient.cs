...
SELECT TGTMNG_OBJ_ID,
       {Settings.Default.CognitecVersion}_FIR_VAL
 ...
 WHERE TGT_MNGGRP_ID = (
   ...
   //+ @"AND FIR_STS_CD = 'S'"
   + $@"AND {Settings.Default.CognitecVersion}_FIR_VAL IS NOT NULL
";
        ...
        public DBScanInfo GetDBScanInfo(string dbscan_id)
        {
            string image_id_Sql = $@"
SELECT IMAGE_ID 
  FROM TB_{Settings.Default.CognitecVersion}_DBSCAN_REQ
 WHERE DBSCAN_ID = :DBSCAN_ID
";

            var DBSCAN_Param = CreateParam("DBSCAN_ID", dbscan_id);

            string image_id = ExecuteScalar(image_id_Sql, DBSCAN_Param) as string;
...
";
        ...
        {
            string sql = $@"
INSERT INTO TB_{Settings.Default.CognitecVersion}_DBSCAN_RSLT (DBSCAN_ID, TGTMNG_OBJ_ID, SCORE, RANK)
...
";
            ...
            sql = $@"
UPDATE TB_{Settings.Default.CognitecVersion}_DBSCAN_REQ
...
SELECT A.TGTMNG_OBJ_ID,
  ...
  FROM TB_TGTMNG_OBJ_INFO A,
       TB_{Settings.Default.CognitecVersion}_DBSCAN_RSLT B
