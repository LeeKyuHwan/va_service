...
namespace FrService
{
    class FrHelper
    {
        ...
        private FrHelper()
        {
            cfg = new Cognitec.FRsdk.Configuration(Settings.Default.config);
            ...
            using (DBClient dbc = new DBClient())
            {
                try
                {
                    ...
                    foreach (Cognitec.FRsdk.Face.Location faceLoc in faceLocs)
                    {
                        ...
                        int size = Convert.ToInt32(faceLoc.boundingBox.width);
                        int width = size;
                        int x = Convert.ToInt32(faceLoc.boundingBox.center.x - width * (1.0 / 2));
                        int height = Convert.ToInt32(width * (4.0 / 3));
                        int y = Convert.ToInt32(faceLoc.boundingBox.center.y - height * (1.0 / 2));
        ...
        private bool setUnitFir(Unit unit)
        {
            ...
            if (faceLocs.Length > 1) faceLocs = faceLocs.OrderByDescending(item => item.boundingBox.width).ToArray();
        ...
        public void resetFir()
        {
            using (DBClient dbc = new DBClient())
            {
                ...
                var rows = dbc.GetResetInfo().Rows;
                int successCount = 0;

                for (int i = 0; i < rows.Count; i++) 
                {
                    var row = rows[i];
                    byte[] imageData = row["IMG_DATA"] as byte[];
                    string id = row["TGTMNG_OBJ_ID"] as string;

                    Cognitec.FRsdk.Image image = loadImage(imageData);
                    Cognitec.FRsdk.Face.Location[] faceLocs = faceFinder.find(image, 0.02f, 0.4f, 0, 0, (int)image.width(), (int)image.height());
                    if (faceLocs.Length == 0) logger.Debug($"FaceNot Found id[{id}]");
                    ...
                    if (eyesLocs.Length == 0)
                    {
                        logger.Debug($"Eyes Not Found id[{id}]");
                        continue;
                    }
                    ...
                    dbc.UpdateFir(id, firData);

                    logger.Debug($"total[{rows.Count}] successCount[{++successCount}]");
                }
            }
        }
    }
