...
namespace DBScanSvc
{
    ...
    public class DBScanServiceBehavior : IDBScanServiceContract
    {
        static ILog logger = LogManager.GetLogger(typeof(DBScanServiceBehavior));
        ...
        public DBScanResult DBScan(Stream stream)
        {
            try
            {
                allowCORS();
                ...
                var data = Utils.ParseFormData(stream);
                logger.Info($"[DBScan] {Utils.PrintDic(data)}");

                if (data.TryGetValue("fir", out dynamic fir) == false)
                    throw new Exception("param fir not found");

                if (data.TryGetValue("threshold", out dynamic threshold) == false)
                    throw new Exception("param threshold not found");

                var result = FrHelper.Instance.DBScan(fir, Convert.ToSingle(threshold));

                logger.Debug($"[DBScan] {result.matches.Count} matches");

                return result;
            }
            catch (Exception ex)
            {
                logger.Error($"[DBScan] {ex.Message}");
