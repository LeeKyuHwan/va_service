...
#define KEY_APNAME _T("SOFTWARE\\CyberLink\\FaceSDKTestbed")
...
#ifndef _WINRT_METRO
    CRegKey reg_key;
    if (reg_key.Open(HKEY_CURRENT_USER, KEY_APNAME, KEY_READ) == ERROR_SUCCESS)
    {
...
#endif
#else
    int nArgs = _vstprintf_s(buffer, _countof(buffer), fmt, marker);
    _TCHAR buffer2[4096];
    _stprintf_s(buffer2, _countof(buffer2), _T("%s %s"), _DBG_MSG_PREFIX_TAG, buffer);
    if(buffer2[_tcslen(buffer2) - 1] != '\n')
        _tcscat_s(buffer2, _countof(buffer2), _T("\n"));
...
LPTSTR ch_TimeFormat(LONGLONG tmCode, LPTSTR szTimeString, int lenStr)
{
...
#else
    _stprintf_s(szTimeString, lenStr, _T("%02d:%02d:%02d"), nHr, nMin, nSec);
#endif
	return szTimeString;
}
...
#else
    struct tm currtime;
    localtime_s(&currtime, &tm_now);
    _stprintf_s(strTime, lenStr, _T("%04d%02d%02d_%02d%02d%02d"),
        currtime.tm_year + 1900, currtime.tm_mon + 1, currtime.tm_mday,
        currtime.tm_hour, currtime.tm_min, currtime.tm_sec);
#endif
}
...
int ch_GetSelectedFolder(HWND hWnd, LPTSTR szFolderOut, int nLenFolderOut)
{
	...
	_tcscpy_s(szFolderOut, nLenFolderOut, szFolder);
